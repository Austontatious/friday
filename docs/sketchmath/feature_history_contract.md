...
Status: passed for the default-off v1 single-sketch extrusion/hole/full-revolve/terminal-fillet rebuild envelope (`SM-FEAT-001`, semantic-reference subset; partial `SM-FEAT-002` through `SM-FEAT-005`)
...
- Unit/kernel coverage proves the prior envelope plus stable vertical-edge identities, fillet recovery/refusal, unique semantic-to-FreeCAD edge resolution, rounded-box volume/bounds/solid validity, and resumable STEP registration.
- API coverage proves default-off document/hole/revolve/fillet gating, preview/commit isolation, disk rehydration, conflict status, geometry/document synchronization, reference recovery, and monotonic feature undo/redo.
- Frontend type-check and all 68 unit tests pass. Targeted Playwright proves fillet create/edit, kernel STEP polling/download, artifact metadata, and reload.
...
- Deterministic layered STL materializes supported vertical extrusion/simple-hole graphs; STEP additionally materializes the documented base-extrusion/terminal-fillet graph. Broader full-graph STEP remains open.
...
- Semantic source/role/signature recovery is implemented for generated extrusion/hole/revolve/fillet-input topology. Fillet artifact execution uniquely reconciles supported edges by endpoints; arbitrary browser picking and general kernel-topology reconciliation remain open.
