...
Typed hole parameters cover simple, counterbore, and countersink style plus through/blind termination. A hole requires one extrusion target and one semantic top-face selector. Rebuild validates conditional geometry, containment, depth, analytic removed volume, and generated topology. The guarded browser creates simple holes and replaces existing holes with all supported styles. Terminal typed holes have native FreeCAD STEP validation; new advanced-hole creation and advanced-hole STL remain outside the envelope.

Typed revolve parameters name a stable sketch axis entity, a finite angle, and new-body/add/cut operation. The current rebuild accepts exactly 360 degrees, rejects profiles that cross the axis, computes analytic Pappus volume and exact full-revolution bounds, and generates source-derived semantic revolved-face references. An axis may be an `axis_2d`, regular line, or construction line. One independent new-body full revolve has validated native FreeCAD STEP materialization. Add/cut remains an analytic signed-volume contract rather than a kernel-validated spatial boolean.
...
For the bounded downstream creation path, the browser attaches to the most recent extrusion's semantic top face. Add remains positive and a one-sided cut is automatically directed negative into that face, matching structural rebuild validation. A terminal cut graph can run through both validated layered STL and kernel STEP jobs. This remains a bounded target policy rather than arbitrary face/body picking.
