...
- With `FRIDAY_SKETCHMATH_PATTERN_FEATURES_ENABLED=1` and `REACT_APP_SKETCHMATH_PATTERN_FEATURES_ENABLED=1`, a terminal built hole exposes feature-level linear pattern count, spacing, and X/Y direction. Count includes the seed. Creation and replacement are canonical feature operations with exact analytic rebuild, undo/redo, and reload; overlap, out-of-target instances, circular pattern, mirror, shell, and pattern artifacts remain outside this guarded slice.
