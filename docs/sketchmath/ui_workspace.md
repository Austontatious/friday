...
- With `FRIDAY_SKETCHMATH_REVOLVE_FEATURES_ENABLED=1` and `REACT_APP_SKETCHMATH_REVOLVE_FEATURES_ENABLED=1`, an empty feature history exposes a full-revolve editor. It creates a new body from the selected closed profile about a chosen construction line, supports stable-axis replacement, and exposes validated STEP through the artifact job lifecycle. Profiles crossing the axis are rejected; partial angles, browser add/cut, and revolve STL remain outside this guarded UI slice.
...
- With `FRIDAY_SKETCHMATH_CHAMFER_FEATURES_ENABLED=1` and `REACT_APP_SKETCHMATH_CHAMFER_FEATURES_ENABLED=1`, the same supported extrusion exposes a distance and `Chamfer outer edges`. It uses the fillet selector discipline, exposes distance replacement, and builds/downloads native kernel STEP.
- With `FRIDAY_SKETCHMATH_PATTERN_FEATURES_ENABLED=1` and `REACT_APP_SKETCHMATH_PATTERN_FEATURES_ENABLED=1`, a terminal built hole exposes feature-level linear count/spacing/X/Y direction and full-circle count/center/direction editors. Count includes the seed. Creation and replacement are canonical feature operations with exact analytic rebuild, undo/redo, and reload; overlap, axis degeneracy, and out-of-target instances are refused. Partial circular arcs, 3D axes, broader seeds, and pattern artifacts remain outside this guarded slice.
- With `FRIDAY_SKETCHMATH_MIRROR_FEATURES_ENABLED=1` and `REACT_APP_SKETCHMATH_MIRROR_FEATURES_ENABLED=1`, a terminal built hole exposes a feature-mirror editor over stable sketch construction lines. The mirror persists that line ID, rebuilds exact reflected geometry, and supports line replacement/history/reload. On-line, overlap, missing-line, and out-of-target cases are refused; plane mirrors, other seeds, and mirror artifacts remain open.
- With `FRIDAY_SKETCHMATH_SHELL_FEATURES_ENABLED=1` and `REACT_APP_SKETCHMATH_SHELL_FEATURES_ENABLED=1`, a supported terminal rectangular new-body extrusion exposes top-open shell thickness. Creation records the semantic top face; replacement preserves feature identity. The terminal shell offers native validated STEP only. Non-rectangular/holed bases, other body operations, multiple openings, and shell STL are refused or left outside the editor.
- The feature-history panel includes a minimum body → sketch/feature model tree. Nodes are selectable; feature selection exposes typed summary properties and a canonical rename action. Normal mode uses names and friendly feature types, while raw IDs stay in Advanced/Debug data. Body/sketch visibility is reported read-only until the renderer can honor edits.
- With `FRIDAY_SKETCHMATH_ARTIFACT_JOBS_ENABLED=1` and `REACT_APP_SKETCHMATH_ARTIFACT_JOBS_ENABLED=1`, the terminal supported body feature exposes Build STL and/or, for bounded solid, revolve, edge-finish, and shell graphs, Build STEP. The panel displays READY/RUNNING/DONE/FAILED, step, input revision, retry, and download; reload recovers the registered artifact from the document.
