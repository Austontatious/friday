...
- With `FRIDAY_SKETCHMATH_ARTIFACT_JOBS_ENABLED=1` and `REACT_APP_SKETCHMATH_ARTIFACT_JOBS_ENABLED=1`, the terminal supported body feature exposes Build STL or, for bounded fillet/chamfer graphs, Build STEP. The panel displays READY/RUNNING/DONE/FAILED, step, input revision, retry, and download; reload recovers the registered artifact from the document.
