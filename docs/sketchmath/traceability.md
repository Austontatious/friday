...
| SM-FEAT-001 canonical feature history | Passed for extrusion/hole/full-revolve/linear-and-circular-hole-pattern rebuild envelope | Immutable IDs, explicit dependencies, revision-checked typed operations, deterministic pure rebuild, structured failure, persistence, guarded UI, feature undo/redo | Later shell/mirror families remain open. |
...
| SM-FEAT-005 fillet/chamfer/pattern/shell | Partial: fillet/chamfer and linear/full-circle hole-pattern subsets | Prior edge-finish checkpoints plus `5973c89`/`3e309de`; stable edge finishing; typed linear/circular patterns with exact volume/bounds/topology, overlap/containment/axis refusal, guarded create/edit, undo/redo/reload | Patterns are analytic and hole-only; partial circular arcs, 3D axes, mirror, shell, pattern artifacts, arbitrary edges, other graphs, and edge-finish STL remain open. |
