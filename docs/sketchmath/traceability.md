...
| SM-FEAT-001 canonical feature history | Passed for extrusion/hole/full-revolve/linear-hole-pattern rebuild envelope | Immutable IDs, explicit dependencies, revision-checked typed operations, deterministic pure rebuild, structured failure, persistence, guarded UI, feature undo/redo | Later shell/circular-pattern/mirror families remain open. |
...
| SM-FEAT-005 fillet/chamfer/pattern/shell | Partial: fillet/chamfer and linear hole-pattern subsets | Prior edge-finish checkpoints plus `5973c89`/`3e309de`; stable convex vertical-edge references, unique kernel matching, validated two-feature and golden eight-feature STEP; typed linear pattern with exact volume/bounds/topology, overlap/containment refusal, guarded create/edit, undo/redo/reload | Linear pattern is analytic and hole-only; circular pattern, mirror, shell, pattern artifacts, arbitrary edges, other graphs, and edge-finish STL remain open. |
