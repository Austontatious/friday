...
| SM-ART artifacts | Passed for supported STL graphs and independent-extrusion/terminal-edge-finish STEP; overall partial | Revision/hash-bound resumable jobs, stale-result refusal, golden STL evidence, validated fillet/chamfer kernel STEP/download; automated cleanup and broader full-graph STEP remain open |
...
| SM-FEAT-005 fillet/chamfer/pattern/shell | Partial: fillet/chamfer subsets | Fillet `1f1b034`/`30af86a`/`53a39ed`; chamfer `24dfc70`/`3be0baa`/`f1b0b2c`; stable convex vertical-edge references, size bounds/recovery, unique kernel matching, validated STEP, guarded property UI and reload | One base extrusion plus one terminal edge finish; arbitrary edges, broader graphs, edge-finish STL, pattern, mirror, and shell remain open. |
...
| SM-ART-001 revisioned artifacts | Passed for supported STL and bounded STEP graphs | Persistent markers, idempotent job ID, revision registration, stale refusal, terminal UI/download, golden STL, validated kernel fillet/chamfer STEP | General full-graph STEP, automated retention/cleanup, cancellation, and broader non-vertical features remain open. |
