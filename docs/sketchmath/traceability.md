...
| SM-FEAT feature history | `SM-FEAT-001` passed; `SM-FEAT-002/003/004/005` partial | Prior extrusion/hole/revolve checkpoints plus `1f1b034`, `30af86a`, `53a39ed`; deterministic fillet contract, live kernel STEP, guarded editable UI, recovery/reload acceptance |
| SM-REF reference stability | Passed for documented generated topology and supported fillet kernel mapping | Stable source/role/adjacency IDs, geometry signatures, exact/recovered states, atomic missing/ambiguous refusal; fillet uniquely maps semantic endpoints to FreeCAD edges without persisting ordinals |
...
| SM-ART artifacts | Passed for supported STL graphs and independent-extrusion/terminal-fillet STEP; overall partial | Revision/hash-bound resumable jobs, stale-result refusal, golden STL evidence, validated fillet kernel STEP/download; automated cleanup and broader full-graph STEP remain open |
...
| SM-FEAT-004 typed holes | Partial | Simple/counterbore/countersink, through/blind, finite conditional validation, containment/breakout/depth checks, semantic top-face recovery, guarded simple-hole UI | Layered STL and browser creation currently support simple holes only. |
| SM-FEAT-005 fillet/chamfer/pattern/shell | Partial: fillet subset only | `1f1b034`, `30af86a`, `e6b272f`, `53a39ed`; convex outer vertical-edge references, radius bounds/recovery, unique kernel endpoint matching, validated STEP, editable guarded UI and live reload | One base extrusion plus terminal fillet only; arbitrary edges, broader graphs, fillet STL, chamfer, pattern, mirror, and shell remain open. |
...
| SM-ART-001 revisioned artifacts | Passed for supported STL and bounded STEP graphs | Persistent markers, idempotent job ID, revision registration, stale refusal, terminal UI/download, golden STL, validated kernel fillet STEP | General full-graph STEP, automated retention/cleanup, cancellation, and broader non-vertical features remain open. |
| SM-QA-002 golden geometry | Passed for mounting-plate v1 envelope | Seven-feature plate/boss/five-hole rebuild, analytic volume/bounds, width-edit recovery, serialization, deterministic watertight layered STL | Fillets and full-graph STEP are absent, so full acceptance scenario 1 remains open. |
