...
Layered STL intentionally refuses counterbore/countersink and shell. Canonical solid STEP supports one positive base followed by positive adds, typed simple/counterbore/countersink holes, or negative top-face cuts; one independent full revolve; and one rectangular top-open base shell. The worker composes exact hole cutters or the exact rectangular cavity and refuses export unless native validity, bounds, and volume match the analytic ledger. Edge-finish STEP remains limited to positive extrusion/simple-hole intermediates. Partial/revolve booleans, advanced-hole/cut before edge finish, patterns/mirrors, and other intermediate types remain rejected.
