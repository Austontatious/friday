...
Status: passed for default-off STL jobs over supported vertical feature graphs; STEP remains limited to one independent extrusion
...
Counterbore/countersink canonical rebuild is implemented, but layered STL for those styles is intentionally refused. Canonical STEP materialization still requires one independent positive one-sided new-body extrusion and a configured FreeCAD worker.
...
- `tests/test_sketchmath_artifact_jobs.py`: READY/RUNNING/DONE/FAILED directory state, idempotent replay, safe resume, revision registration, and stale-result refusal.
- `tests/test_sketchmath_feature_artifact.py` and `tests/test_sketchmath_stl_export.py`: safe revisioned paths, deterministic STL, hole-aware volume, and mesh closure.
...
Implementation checkpoints: `718ee6b`, `0ef759c`, `9860570`, `217e741`, `b88b1be`, `a3d705c`, and `0979dfe`.
