...
class SummariesStore:
    ...
    def load(self, user_id: str, workspace_id: str, thread_id: str = "default") -> Dict[str, Any]:
        path = self._path(user_id, workspace_id, thread_id)
        if not path.exists():
    ...
    def save(self, user_id: str, workspace_id: str, data: Dict[str, Any], thread_id: str = "default") -> None:
        path = self._path(user_id, workspace_id, thread_id)
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding="utf-8")

    def increment_turns(self, user_id: str, workspace_id: str, count: int = 1, thread_id: str = "default") -> None:
        data = self.load(user_id, workspace_id, thread_id)
        data["turns_since_summary"] = int(data.get("turns_since_summary", 0)) + count
        self.save(user_id, workspace_id, data, thread_id)
    ...
    def add_open_loop(self, user_id: str, workspace_id: str, item: str, thread_id: str = "default") -> Dict[str, Any]:
        data = self.load(user_id, workspace_id, thread_id)
        loop = {
            "id": self._new_id(),
            "item": item,
    ...
    def resolve_open_loop(self, user_id: str, workspace_id: str, loop_id: str, thread_id: str = "default") -> bool:
        data = self.load(user_id, workspace_id, thread_id)
        updated = False
    ...
    def add_task(self, user_id: str, workspace_id: str, title: str, notes: Optional[str] = None, thread_id: str = "default") -> Dict[str, Any]:
        data = self.load(user_id, workspace_id, thread_id)
        task = {
            "id": self._new_id(),
            "title": title,
    ...
    def list_open_loops(self, user_id: str, workspace_id: str, thread_id: str = "default") -> List[Dict[str, Any]]:
        data = self.load(user_id, workspace_id, thread_id)
        return [loop for loop in data.get("open_loops", []) if loop.get("status") != "resolved"]

    def list_tasks(self, user_id: str, workspace_id: str, status: Optional[str] = None, thread_id: str = "default") -> List[Dict[str, Any]]:
        data = self.load(user_id, workspace_id, thread_id)
    ...
    def list_commitments(self, user_id: str, workspace_id: str, thread_id: str = "default") -> List[Dict[str, Any]]:
        data = self.load(user_id, workspace_id, thread_id)
        return data.get("commitments", [])

    def search(self, user_id: str, workspace_id: str, query: str, limit: int = 5) -> List[Dict[str, Any]]:
        data = self.load(user_id, workspace_id)
    ...
    def _path(self, user_id: str, workspace_id: str, thread_id: str) -> Path:
        ...
        return Path(self._data_dir) / "users" / safe_id / "workspaces" / safe_ws / "summaries" / f"{safe_thread}.json"
