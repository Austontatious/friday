...
def run_tool(tool_name: str, args: dict = None) -> dict:
    """Run a registered tool and return structured result.
    Returns: {
        "success": bool,
        "output": Any (stringified),
        "error": str | None
    }
    """
    from .registry import get_tool, validate_tool_args
    import logging
    ...
    try:
        ...
        result = tool.entrypoint(**args)
        logger.info("✅ Tool %s executed successfully", tool.name)
        return {"success": True, "output": result, "error": None}
    except Exception as exc:
        logger.error("❌ Tool %s failed: %s", tool_name, exc, exc_info=True)
        return {"success": False, "output": None, "error": f"{exc}"}
